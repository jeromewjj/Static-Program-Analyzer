...
// ai-gen start (gpt3, 3)
TEST_CASE("INDIVIDUAL_NODE: Should extract constant correctly", "[ExtractEntitytVisitor]") {
    ...
    SECTION("should extract") {
        ...
        // expected
        std::unordered_set<std::string> EXPECTED_CONSTS{"3"};

        // assert
        REQUIRE(e_v.getConsts() == EXPECTED_CONSTS);
    }
    ...
    SECTION("should extract despite generic type; dynamic dispatch / runtime polymorphism") {
        ...
        ExtractEntityVisitor e_v{pkbAdapter};
        ASTNodePtr upcasted_const3 = std::make_shared<ConstantNode>("3");
        upcasted_const3->acceptVisitor(e_v);
    ...
    SECTION("should not extract") {
        ...
        // expected
        std::unordered_set<std::string> EXPECTED_CONSTS{};
